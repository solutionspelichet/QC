...
/* ---------- KO: affichage/masquage des blocs ---------- */
...
let CONFIG = { WEBAPP_BASE_URL: '' }; // peut être rempli par config.json
  ...
  qsa('.qc-form').forEach(form=>{
    form.addEventListener('submit', async (ev)=>{
      ...
      for (const q of questions) {
        ...
        // KO ⇒ photo(s) + commentaire obligatoires
        let photo = null;                // rétro-compat: première photo
        let photos = [];                 // nouveau: liste de photos
        let commentaire = '';
        ...
        if (val === 'KO') {
          ...
          // multi-photos support (si <input multiple> en HTML)
          photos = await filesToDataUrlsWithExt(fileInput);
          if (photos.length > 0) photo = photos[0]; // compat backend existant
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  // Hooks Caméra/Galerie -> cibles (cohérents avec l’index.html “Solution A”)
...
async function filesToDataUrlsWithExt(input){
  // Supporte 0, 1, N fichiers (si <input multiple> en HTML)
  if (!input || !input.files || input.files.length===0) return [];
  const out = [];
  for (const file of input.files) {
    const ext = (file.name.split('.').pop()||'').toLowerCase();
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  hookCameraGalleryPair('btnPdPhotoCam','pd_photo_cam','btnPdPhotoGal','pd_photo_gal','pd_photo_target','pd_photo_label');
  hookCameraGalleryPair('btnPdBcCam','pd_bc_cam','pdBcGal','pd_bc_gal','pd_barcode_file','pd_bc_label'); // si btn id 'pdBcGal' différent, ajuste-le dans index.html
});  const meta=document.querySelector('meta[name="theme-color"]');
  if(meta)meta.setAttribute('content',theme==='dark'?'#0f1115':'#FF6A00');
}
